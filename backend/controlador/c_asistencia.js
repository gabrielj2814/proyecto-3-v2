...
AsistenciaControlador.consultarTodasLasAsistenciasControlador= async (req,res) => {
    var respuesta_api={asistencias:[],mensaje:"",estado_peticion:""}
...
AsistenciaControlador.agregarObservacionAsistencia= async (req,res,next) => {
    var respuesta_api={mensaje:"",estado_peticion:""}
    const {asistencia} = req.body
    let asistencia_modelo=new AsistenciaModelo()
    let result_asistencia=await asistencia_modelo.agregarObservacion(asistencia.id_asistencia,asistencia.observacion_asistencia)
    if(result_asistencia.rowCount>0){
        respuesta_api.mensaje="observacion agregada con exito"
        respuesta_api.estado_peticion="200"
        // res.writeHead(200,{"Content-Type":"application/json"})
        // res.write(JSON.stringify(respuesta_api))
        // res.end()
        req.vitacora=VitacoraControlador.json(respuesta_api,asistencia.token,"UPDATE","tasistencia",asistencia.id_asistencia)
        next()
    }
    else{
        respuesta_api.mensaje="no se pudo agregar la observación (no se encontro la asistencia del trabajador)"
        respuesta_api.estado_peticion="404"
        res.writeHead(200,{"Content-Type":"application/json"})
        res.write(JSON.stringify(respuesta_api))
        res.end()
