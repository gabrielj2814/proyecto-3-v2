...
ControladorPromocion.consultarPromocionProfesor= async (req,res) => {
    const respuesta_api={mensaje:"",datos:[],estado_respuesta:false,color_alerta:""}
    const {cedula} = req.params
    let modeloPromocion = new ModeloPromocion()
    const resultPromocion=await modeloPromocion.consultarPromocionProfesor(cedula)
    if(resultPromocion.rowCount>0){
        respuesta_api.mensaje="consula completada"
        respuesta_api.estado_respuesta=true
        respuesta_api.color_alerta="success"
...
ControladorPromocion.consultarPorInscripcion= async (req,res,next) => {
    const respuesta_api={mensaje:"",datos:[],estado_respuesta:false,color_alerta:""}
    const {id,token} = req.params
    let modeloPromocion = new ModeloPromocion()
