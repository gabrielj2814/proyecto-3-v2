...
class ComponentRepresentante extends React.Component{
  constructor(){
    ...
    this.consultarTodosLosRepresentantes = this.consultarTodosLosRepresentantes.bind(this)
  ...
  async UNSAFE_componentWillMount(){
    ...
    if(acessoModulo){
      var json_server_response=await this.consultarTodosLosRepresentantes();
  ...
  async buscar(a){
    ...
    if(valor!==""){
      await axios.get(`http://${servidor.ipServidor}:${servidor.servidorNode.puerto}/configuracion/representate/consultar-patron/${valor}`)
        .then(respuesta=>{
  ...
  redirigirFormulario(a){
    ...
    if(input.value==="Registrar"){
      this.props.history.push("/dashboard/configuracion/representate/registrar")
    }
  }

  actualizarElementoTabla(a){
      var input=a.target;
      this.props.history.push("/dashboard/configuracion/representate/actualizar/"+input.id);
  }

  consultarElementoTabla(a){
      let input=a.target;
      this.props.history.push("/dashboard/configuracion/representate/consultar/"+input.id);
  }
  ...
  const jsx_tabla_body=(
        <tbody>
          {this.state.registros.map((representate)=>{
            return(
              <tr key={representante.cedula_representante}>
                <td>{representante.cedula_representante}</td>
                <td>{representante.nombres_representante}</td>
                <td>{representante.apellidos_representante}</td>
                <td>{ (representante.sexo_representante == '1') ? "Masculino" : "Femenino" }</td>
                {!representante.vacio &&
                  <td>
                     <ButtonIcon
    ...
    return (
      <div className="component_trabajador_inicio">
          <ComponentDashboard
          componente={jsx_representante_inicio}
