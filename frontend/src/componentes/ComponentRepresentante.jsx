...
class ComponentRepresentante extends React.Component{
  ...
  async validarAccesoDelModulo(modulo,subModulo){
    ...
    async buscar(a){
      ...
      if(valor!==""){
        await axios.get(`http://${servidor.ipServidor}:${servidor.servidorNode.puerto}/configuracion/representante/consultar-patron/${valor}`)
          .then(respuesta=>{
      ...
      <>
        <div className="row justify-content-end  mb-3">
            <div className="col-auto">
                <button className="btn btn-info"  data-toggle="modal" onClick={this.mostrarModalAyuda}>
  ...
  const jsx_tabla_body=(
        <tbody>
          { this.state.registros.map((representante)=>{
            return(
              <tr key={representante.id_cedula_representante}>
              <td>{representante.id_cedula_representante}</td>
              <td>{representante.nombres_representante}</td>
              <td>{representante.apellidos_representante}</td>
              <td>{representante.numero_hijos_representante}</td>
              {!representante.vacio &&
                <td>
                    <ButtonIcon
                      clasesBoton="btn btn-warning btn-block"
                      value={representante.id_cedula_representante}
                      id={representante.id_cedula_representante}
    ...
    var jsx_representante_inicio=(
      <div>
        ...
        <TituloModulo clasesrow="row" clasesColumna="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 text-center" tituloModulo="Módulo Representante"/>
...
export default withRouter(ComponentRepresentante)
