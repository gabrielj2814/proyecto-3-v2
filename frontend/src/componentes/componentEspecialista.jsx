...
class ComponentEspecialista extends React.Component {
    ...
    async componentWillMount(){
        let acessoModulo=await this.validarAccesoDelModulo("/dashboard/configuracion","/especialista")
        if(acessoModulo){
    ...
    async consultarEspecialistas(){
        // const token=localStorage.getItem('usuario')
        await axiosCustom.get(`configuracion/especialista/consultar-todos`)
        .then(respuesta => {
    ...
    redirigirFormulario(a){
        this.props.history.push("/dashboard/configuracion/especialista/registrar")
    }

    irAlFormularioDeActualizacion(a){
        let input=a.target
        this.props.history.push(`/dashboard/configuracion/especialista/actualizar/${input.id}`)
    }
    ...
    render(){
        ...
        const jsx_tabla_encabezado=(
            <thead>
                <tr>
                    <th>Cédula </th>
                    <th>Nombre del Profesor Especialista</th>
                    <th>Especialidad</th>
                    <th>Estatus</th>
                </tr>
            </thead>
        ...
        const jsx_tabla_body=(
            <tbody>
                {this.state.registros.map((especialista,index)=>{
                    return(
                        <tr key={index}>
                            <td>{especialista.id_cedula}</td>
                            <td>{especialista.nombres} {especialista.apellidos}</td>
                            <td>{especialista.especialidad}</td>
                            <td>{(especialista.estatus_especialista==="1")?"Activo":"Inactivo"}</td>
                            <td>
                                <ButtonIcon
        ...
        const jsx=(
            <div>
                ...
                <TituloModulo clasesRow="row mb-5" clasesColumna="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 text-center" tituloModulo="Módulo Especialista"/>
...
export default withRouter(ComponentEspecialista)
