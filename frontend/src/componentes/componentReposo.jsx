...
import '../css/componentReposo.css'
//componentes
...
class ComponentReposo extends React.Component{
    ...
    async UNSAFE_componentWillMount(){
        var json_server_response=await this.consultarTodosReposo();
    ...
    actualizarElementoTabla(a){
        var input=a.target;
        this.props.history.push("/dashboard/configuracion/reposo/actualizar/"+input.id);
    }
        ...
        if(valor!==""){
          await axios.get(`http://localhost:8080/configuracion/reposo/consultar-patron/${valor}`)
            .then(respuesta=>{
              ...
              this.setState({registros:respuesta_servidor.reposos})
            })
            .catch(error=>{
              console.log(error)
    ...
    async escribir_codigo(a){
        ...
        if(valor!==""){
          ...
          .then(respuesta=>{
            respuesta_servidor=respuesta.data
            console.log(respuesta_servidor)
            this.setState({datoDeBusqueda:valor,registros:respuesta_servidor.reposos})
          })
          .catch(error=>{
            console.log(error)
            alert("error en el servidor")
    ...
    render(){
        ...
        const jsx_tabla_encabezado=(
          ...
          const jsx_tabla_body=(
            <tbody>
                  {this.state.registros.map((reposo)=>{
                      return(
                          <tr key={reposo.id_reposo}>
                            <td>{reposo.id_reposo}</td>
                            <td>{reposo.nombre_reposo}</td>
                          {!reposo.vacio &&
                              <td>
                                  <ButtonIcon 
          ...
          var jsx_reposo_inicio=(
            <div>
                ...
                <TituloModulo clasesrow="row" clasesColumna="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 text-center" tituloModulo="Modulo de Reposo"/>
                <ComponentTablaDatos 
        ...
        return(
            <div className="component_reposo_inicio">
                <ComponentDashboard
                componente={jsx_reposo_inicio}
...
export default withRouter(ComponentReposo)