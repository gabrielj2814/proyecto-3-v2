...
import '../css/componentTipoTrab.css'
//componentes
...
class ComponentTipoTrab extends React.Component{
  ...
  async UNSAFE_componentWillMount(){
    ...
    if(acessoModulo){
          var json_server_response=await this.consultarTodosTipoTrabajador();
    ...
    verficarLista(json_server_response){
        if(json_server_response.length===0){
            json_server_response.push({
              ...
              descripcion_tipo_trabajador:"vacio",
              vacio:"vacio"
            })
    ...
    actualizarElementoTabla(a){
        var input=a.target;
        this.props.history.push("/dashboard/configuracion/tipo-trabajador/actualizar/"+input.id);
    }
    
    consultarElementoTabla(a){
        let input=a.target;
        //alert("Consultar -> "+input.id);
        this.props.history.push("/dashboard/configuracion/tipo-trabajador/consultar/"+input.id);
    }
      ...
      if(valor!==""){
        await axios.get(`http://${servidor.ipServidor}:${servidor.servidorNode.puerto}/configuracion/tipo-trabajador/consultar-tipos-trabajador-patron/${valor}`)
          .then(respuesta=>{
            ...
            this.setState({registros:respuesta_servidor.tipos_trabajadores})
          })
          .catch(error=>{
            console.log(error)
    ...
    async escribir_codigo(a){
      ...
      if(valor!==""){
        ...
        .then(respuesta=>{
          respuesta_servidor=respuesta.data
          console.log(respuesta_servidor)
          this.setState({datoDeBusqueda:valor,registros:respuesta_servidor.tipos_trabajadores})
        })
        .catch(error=>{
          console.log(error)
          alert("error en el servidor")
    ...
    render(){
        const jsx_tabla_encabezado=(
          ...
          const jsx_tabla_body=(
            <tbody>
                  {this.state.registros.map((tipo_trabajador)=>{
                      return(
                          <tr key={tipo_trabajador.id_tipo_trabajador}>
                            <td>{tipo_trabajador.id_tipo_trabajador}</td>
                            <td>{tipo_trabajador.descripcion_tipo_trabajador}</td>
                           {!tipo_trabajador.vacio &&
                              <td>
                                  <ButtonIcon 
        ...
        var jsx_tipo_trabajador_inicio=(
            <div>
        ...
        return(
            <div className="component_tipo_trabajador_inicio">
               <ComponentDashboard
                componente={jsx_tipo_trabajador_inicio}
...
export default withRouter(ComponentTipoTrab);