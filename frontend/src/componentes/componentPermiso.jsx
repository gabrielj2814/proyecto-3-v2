...
import '../css/componentPermiso.css'
//componentes
...
class ComponentPermiso extends React.Component{
    ...
    async UNSAFE_componentWillMount(){
      ...
      if(acessoModulo){
          var json_server_response=await this.consultarTodosPermiso();
    ...
    async consultarTodosPermiso(){
        ...
        .then(respuesta=>{
          respuesta_servidor=respuesta.data.permisos
    ...
    actualizarElementoTabla(a){
        var input=a.target;
        this.props.history.push("/dashboard/configuracion/permiso/actualizar/"+input.id);
    }
    
    consultarElementoTabla(a){
        let input=a.target;
        //alert("Consultar -> "+input.id);
        this.props.history.push("/dashboard/configuracion/permiso/consultar/"+input.id);
    }
    ...
    async buscar(a){
        ...
        if(valor!==""){
          await axios.get(`http://${servidor.ipServidor}:${servidor.servidorNode.puerto}/configuracion/permiso/consultar-perfiles-patron/${valor}`)
            .then(respuesta=>{
              respuesta_servidor=respuesta.data
              console.log(respuesta_servidor)
              this.setState({registros:respuesta_servidor.permisos})
            })
            .catch(error=>{
              console.log(error)
      ...
      async escribir_codigo(a){
        ...
        if(valor!==""){
          ...
          .then(respuesta=>{
            respuesta_servidor=respuesta.data
            console.log(respuesta_servidor)
            this.setState({datoDeBusqueda:valor,registros:respuesta_servidor.permisos})
          })
          .catch(error=>{
            console.log(error)
            alert("Error en el servidor")
          ...
          const jsx_tabla_body=(
            <tbody>
                  {this.state.registros.map((perfil)=>{
                      return(
                          <tr key={perfil.id_permiso}>
                            <td>{perfil.id_permiso}</td>
                            <td>{perfil.nombre_permiso}</td>
                          {!perfil.vacio &&
                              <td>
                                  <ButtonIcon 
...
export default withRouter(ComponentPermiso)